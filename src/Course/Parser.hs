...
list ::
  Parser a
  -> Parser (List a)
list p = bindParser (flmapParser (list p) . (:.)) p ||| valueParser Nil
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy f = flbindParser character $ \c ->
  if f c
    then pure c
...
natural ::
  Parser Int
natural = ((?? failed) . (pure <$>) . read) =<< list1 digit
...
surnameParser ::
  Parser Chars
surnameParser = lift2 (:.) upper rest
  where rest = lift2 (++) (thisMany 5 lower) (list lower)
