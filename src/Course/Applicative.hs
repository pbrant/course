...
replicateA i fa =
  if i <= 0
    then pure Nil
    else lift2 (:.) fa $ replicateA (i-1) fa
...
filtering ::
  ...
  -> List a
  -> f (List a)
filtering f = foldRight (\a -> lift2 (prependIfTrue a) (f a)) (pure Nil)
  where prependIfTrue a b l = if b then a :. l else l
