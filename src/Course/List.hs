...
filter ::
  ...
  -> List a
  -> List a
filter f = foldRight (\ x r -> if f x then x :. r else r) Nil
...
-- []
--
-- x prop> let types = x :: List Int in notReverse x ++ notReverse y == notReverse (y ++ x)
--
-- x prop> let types = x :: Int in notReverse (x :. Nil) == x :. Nil
notReverse ::
  List a
  -> List a
notReverse _ = Nil
