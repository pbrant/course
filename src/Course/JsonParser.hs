...
jsonString = between
  (is '"')
  (charTok '"')
  (list (noneof "\"\\" ||| (is '\\' *> (hexu ||| specialChar))))
  where specialChar = character >>= (\c ->
          ((pure . fromSpecialCharacter) <$> toSpecialCharacter c) ?? failed)

(<+/>) :: Parser Char -> Parser Chars -> Parser Chars
(<+/>) = lift2 (:.)

(<++/>) :: Parser Char -> Parser Char -> Parser Chars
(<++/>) = lift2 (\ a b -> a :. b :. Nil)

(<++>) :: Parser Chars -> Parser Chars -> Parser Chars
(<++>) = lift2 (++)
...
jsonNumber ::
  Parser Rational
jsonNumber = number >>= (\s -> (pure <$> readFloat s) ?? failed)
  where
    number = sign <++> int <++> ((frac <++> exp) ||| frac ||| exp ||| valueParser "")
    e = oneof "eE" <+/> sign
    exp = e <++> digits1
    frac = is '.' <+/> digits1
    digit19 = oneof "123456789"
    sign = string "-" ||| (string "+" *> valueParser "") ||| valueParser ""
    int = (digit19 <+/> list digit) ||| (pure <$> digit)
...
jsonObject ::
  Parser Assoc
jsonObject = betweenSepbyComma '{' '}' (lift2 (,) (jsonString <* charTok ':') jsonValue)
