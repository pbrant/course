...
findLeft f (ListZipper l c r) =
  let (toRight, toLeft) = break f l
  in case toLeft of
    Nil -> IsNotZ
    (x :. xs) -> IsZ (ListZipper xs x (toRight ++ c :. r))
...
findRight f (ListZipper l c r) =
  let (toLeft, toRight) = break f r
  in case toRight of
    Nil -> IsNotZ
    (x :. xs) -> IsZ (ListZipper (reverse toLeft ++ c :. l) x xs)
...
moveLeftLoop (ListZipper Nil c r) =
  case reverse (c :. r) of
    Nil -> undefined -- oh where, oh where is my non-empty list
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper l c (x :. xs)) = ListZipper (c :. l) x xs
...
swapLeft (ListZipper (x :. xs) c r) = IsZ (ListZipper (c :. xs) x r)
...
swapRight (ListZipper l c (x :. xs)) = IsZ (ListZipper l x (c :. xs))
...
moveLeftN i z
  | i == 0 = IsZ z
  | i < 0 = moveRightN (i*(-1)) z
-- Ack, see answer...
  | otherwise = (moveLeftN (i-1) <$> toOptional (moveLeft z)) ?? IsNotZ
...
moveRightN i z
  ...
  | i < 0 = moveLeftN (i*(-1)) z
-- Ack, see answer...
  | otherwise = (moveRightN (i-1) <$> toOptional (moveRight z)) ?? IsNotZ
...
moveLeftN' i z@(ListZipper l _ _) =
  if i < 0
    then moveRightN' (negate i) z
    else (Right <$> toOptional (moveLeftN i z)) ?? Left (length l)
...
moveRightN' ::
  ...
  -> ListZipper a
  -> Either Int (ListZipper a)
moveRightN' i z@(ListZipper _ _ r) =
  if i < 0
    then moveLeftN' (negate i) z
    else (Right <$> toOptional (moveRightN i z)) ?? Left (length r)
...
-- prop> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end (ListZipper l c r) =
  case reverse r ++ (c :. l) of
    Nil -> undefined
    (x :. xs) -> ListZipper xs x Nil
...
start ::
  ListZipper a
  -> ListZipper a
start (ListZipper l c r) =
  case reverse l ++ (c :. r) of
    Nil -> undefined
    (x :. xs) -> ListZipper Nil x xs
...
-- /Tip:/ Use `zipWith`
  (ListZipper lf cf rf) <*> (ListZipper l c r) =
    ListZipper (zipWith ($) lf l) (cf c) (zipWith ($) rf r)
...
instance Traversable ListZipper where
  traverse f (ListZipper l c r) =
    ListZipper <$> traverse f l <*> f c <*> traverse f r
